...
public class BaseNCodecTest {
	...
	public static BaseNCodec mockBaseNCodec1() {
		BaseNCodec mockInstance = mock(BaseNCodec.class,
				withSettings().useConstructor(0, 0, 0, 0).defaultAnswer(Mockito.CALLS_REAL_METHODS));
		return mockInstance;
	}
	...
	/**
	 * Test the Context string representation has debugging info. This is not part
	 * of the API and the test should be changed if the string format is updated.
	 */
	@Test
...
//    }
	...
	public void testEnsureBufferSize() {
		...
		Assert.assertEquals("buffer should be initialised to default size", ncodec.getDefaultBufferSize(),
		...
		Assert.assertEquals("buffer should not expand unless required", ncodec.getDefaultBufferSize(),
				context.buffer.length);
		...
		// Hits the edge case where the required capacity is more than the default
		// expansion.
	 ...
	 *
	 * <p>
	 * Note: If the buffer is required to expand to above the max buffer size it may
	 * not work on all VMs and may have to be annotated with @Ignore.
	 * </p>
	 */
	@Test
	...
	private static void assertEnsureBufferSizeExpandsToMaxBufferSize(final boolean exceedMaxBufferSize) {
		...
		// Using a buffer that must be doubled to get close to 2GiB requires at least
		// 3GiB
	...
	/**
	 * Verify this VM can allocate the given size byte array. Otherwise skip the
	 * test.
	 */
	...
	/**
	 * Gets the presumable free memory; an estimate of the amount of memory that
	 * could be allocated.
	 *
	 * <p>
	 * This performs a garbage clean-up and the obtains the presumed amount of free
	 * memory that can be allocated in this VM. This is computed as:
	 * <p>
	 *
	 * <pre>
	 * System.gc();
	 ...
	 * @see <a href="https://stackoverflow.com/a/18366283"> Christian Fries
	 *      StackOverflow answer on Java available memory</a>
	 */
